...
class SupplierPaymentAllocationDao
  ...
  Future<int> insertAllocation(
    SupplierPaymentAllocationsCompanion allocation,
  ) {
  ...
  Future<SupplierPaymentAllocation?> getAllocationById(
    int id,
  ) {
  ...
  Future<List<SupplierPaymentAllocation>>
      getAllocationsForPayment(
    int paymentId,
  ) {
    return (select(supplierPaymentAllocations)
          ..where(
            (a) => a.paymentId.equals(paymentId),
          )
          ..orderBy([
            (a) => OrderingTerm(
                  expression: a.id,
  ...
  Future<List<SupplierPaymentAllocation>>
      getAllocationsForDelivery(
    int deliveryId,
  ) {
    return (select(supplierPaymentAllocations)
          ..where(
            (a) => a.deliveryId.equals(deliveryId),
          )
          ..orderBy([
            (a) => OrderingTerm(
                  expression: a.id,
  ...
  Future<double> getAllocatedAmountForPayment(
    int paymentId,
  ) async {
  ...
  Future<double> getAllocatedAmountForDelivery(
    int deliveryId,
  ) async {
    final result = await customSelect(
      '''
  ...
  Future<double> getPaymentUnallocatedAmount(
    int paymentId,
  ) async {
    final payment = await (select(supplierPayments)
          ..where((p) => p.id.equals(paymentId)))
        .getSingleOrNull();

    if (payment == null) {
      throw Exception(
        'Supplier payment with ID $paymentId was not found.',
      );
    }

    final allocated =
        await getAllocatedAmountForPayment(paymentId);
  ...
  Future<double> getDeliveryOutstandingAmount(
    int deliveryId,
  ) async {
    final delivery =
        await (select(supplierDeliveries)
              ..where(
                (d) => d.id.equals(deliveryId),
              ))
            .getSingleOrNull();

    if (delivery == null) {
      throw Exception(
        'Supplier delivery with ID $deliveryId was not found.',
      );
    }

    final allocated =
        await getAllocatedAmountForDelivery(
      deliveryId,
    );

    final outstanding =
        delivery.totalAmount - allocated;
  ...
  Future<int> allocatePayment({
    ...
    return transaction(() async {
      ...
      if (payment.supplierId != delivery.supplierId) {
        throw Exception(
          'Payment and delivery belong to different suppliers.',
        );
      }
      ...
      final allocatedToPayment =
          await getAllocatedAmountForPayment(
        paymentId,
      );

      final paymentRemaining =
          payment.amount - allocatedToPayment;
      ...
      final allocatedToDelivery =
          await getAllocatedAmountForDelivery(
        deliveryId,
      );

      final deliveryRemaining =
          delivery.totalAmount - allocatedToDelivery;
  ...
  Future<bool> updateAllocation({
    ...
    return transaction(() async {
      ...
      final payment =
          await (select(supplierPayments)
                ..where(
                  (p) => p.id.equals(existing.paymentId),
                ))
              .getSingleOrNull();
      ...
      final delivery =
          await (select(supplierDeliveries)
                ..where(
                  (d) => d.id.equals(existing.deliveryId),
                ))
              .getSingleOrNull();
      ...
      final paymentRemaining =
          payment.amount -
              paymentAllocatedExcludingCurrent;
      ...
      final deliveryRemaining =
          delivery.totalAmount -
              deliveryAllocatedExcludingCurrent;
  ...
  Future<int> deleteAllocation(
    int allocationId,
  ) {
    return (delete(supplierPaymentAllocations)
          ..where(
            (a) => a.id.equals(allocationId),
          ))
        .go();
  ...
  Future<int> deleteAllocationsForPayment(
    int paymentId,
  ) {
    return (delete(supplierPaymentAllocations)
          ..where(
  ...
  Future<int> deleteAllocationsForDelivery(
    int deliveryId,
  ) {
    return (delete(supplierPaymentAllocations)
          ..where(
  ...
  Future<List<SupplierPaymentAllocationWithDelivery>>
      getPaymentAllocationsWithDeliveries(
    int paymentId,
  ) async {
    final query =
    ...
    return query.map((row) {
      return SupplierPaymentAllocationWithDelivery(
        allocation:
            row.readTable(supplierPaymentAllocations),
        delivery:
            row.readTable(supplierDeliveries),
      );
    }).get();
  }
  ...
  Future<List<SupplierPaymentAllocationWithPayment>>
      getDeliveryAllocationsWithPayments(
    int deliveryId,
  ) async {
    final query =
        select(supplierPaymentAllocations).join([
    ...
    final query =
      ...
      innerJoin(
        ...
        supplierPayments.id.equalsExp(
          ...
          ..where(
            supplierPayments.supplierId.equals(
              supplierId,
            ),
          )
          ..orderBy([
