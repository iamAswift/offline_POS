...
class _UserProfileScreenState
  ...
  final _guarantorPhoneController =
      TextEditingController();
  final _salaryController = TextEditingController();
  final _amountOwedController =
      TextEditingController();
  ...
  Future<void> _loadProfile() async {
    try {
      ...
      final profile =
          await profileDao.getProfileByUserId(
        widget.userId,
      );
      ...
      if (profile != null) {
        ...
        _guarantorPhoneController.text =
            profile.guarantorPhone ?? '';
        _salaryController.text =
            profile.salary.toString();
        _amountOwedController.text =
            profile.amountOwed.toString();

        _canReceiveStock =
            profile.canReceiveStock;
        _canCountStock =
            profile.canCountStock;
      }

      setState(() {});
    } catch (e) {
      debugPrint(
        'Error loading employee profile: $e',
      );
    }
  }
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      ...
      await profileDao.upsertProfile(
        UserProfilesCompanion.insert(
          ...
          guarantorPhone: Value(
            _guarantorPhoneController.text.trim(),
          ),
          ...
          amountOwed: Value(
            double.tryParse(
                  _amountOwedController.text.trim(),
                ) ??
                0.0,
          ),
      ...
      debugPrint(
        'Error saving employee profile: $e',
      );
